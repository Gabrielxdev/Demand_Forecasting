...
@st.cache_resource # Cache the model and encoders to avoid reloading them on every interaction

def load_artifacts():
    current_dir = os.path.dirname(os.path.abspath(__file__))
    with open(os.path.join(current_dir, "xgboost_demand_model.pkl"), "rb") as f:
        model = pickle.load(f)
